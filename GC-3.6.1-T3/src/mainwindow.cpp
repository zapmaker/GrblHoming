...
/// LETARTARE T3
    connect(ui->Check,SIGNAL(clicked()),this,SLOT(checkCode()));
...
/// LETARTARE  T2
	...
	connect(this, SIGNAL(setLinesFile(QString)), ui->outputLines, SLOT(setText(QString)));
/// T3
...
void MainWindow::preProcessFile(QString filepath)
{
    ...
    {
        posList.clear();
/// LETARTARE  T2 'int totalLinesFile' -> global
        totalLinesFile = 0;
        ...
        if (totalLinesFile == 0)
            totalLinesFile = 1;
/// LETARTARE  T2
        ui->outputLines->setText(QString().setNum(totalLinesFile));
...
/// LETARTARE  T3  called by pushbutton 'Check'
void MainWindow::checkCode()
{
    checkState = ui->Check->isChecked();
    /// send '$C' to Grbl
